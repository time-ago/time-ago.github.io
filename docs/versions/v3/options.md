...
# Options
The `Parse()` function is a [variadic function](https://en.wikipedia.org/wiki/Variadic_function) that accepts an indefinite number of string arguments after the first argument. It means that you can pass as many options as you want to modify the output.
...
func main() {
    ...
    out, err := ago.Parse(tenSecondsAgo, ago.OptOnline)
...
func main() {
    ...
    out, err := ago.Parse(tenSecondsAgo, ago.OptJustNow, ago.OptNoSuffix)
...
| --- | --- | --- |
| `OptOnline`<br>`"online"` | Displays **Online** if the date interval is within 60 seconds. For example instead of `13 seconds ago` prints `Online` | It could be used when you need to display the online status of users, to show that the user is `Online` when the date interval is within a certain threshold. |
| `OptJustNow`<br>`"justNow"` | Displays **Just now** if date interval withing 60 seconds. For example instead of `32 seconds ago` prints `Just now` | It could be used when you need to display the `Just now` message when something was sent, done, published, etc. within a certain threshold. |
